...
def generate_mask(image, box, rect):
    ...
    corner_pointx2, corner_pointy2 = box[1]
    corner_pointx3, corner_pointy3 = box[2]
    corner_pointx4, corner_pointy4 = box[3]
    a = max(0, corner_pointy1)
    b = max(0, corner_pointy2)
    c = max(0, corner_pointy3)
    d = max(0, corner_pointy4)
    y1 = min(a,b,c,d)
    y2 = max(a, b, c, d)

    x1 = max(0, min(corner_pointx1, corner_pointx2, corner_pointx3, corner_pointx4))
    x2 = max(0, max(corner_pointx1, corner_pointx2, corner_pointx3, corner_pointx4))
...
for i, cnt in enumerate(contours):
        ...
        if cur_area > area_limt:
            ...
            box = np.int0(box)
            #Generate mask method creates slightly bigger bounding box and slightly less blurry
            iso_img = generate_mask(image, box, rect)
            #isolate image creates perfect fit bbox but can be a little more blurry
